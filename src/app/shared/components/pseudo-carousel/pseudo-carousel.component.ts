...
@Component({
  selector: 'app-pseudo-carousel',
  templateUrl: './pseudo-carousel.component.html',
  styleUrl: './pseudo-carousel.component.scss',
  standalone: false,
})
export class PseudoCarouselComponent {
  ...
  indexToFront(index: any) {
    ...
    if (this.animates[index] != 0) {
      ...
      if (pos) {
        ...
        // Check if mov and mov2 are undefined
        if (mov && mov2) {
          const direction =
          ...
          console.error('mov or mov2 is undefined');
        }
      }
    }
  ...
  animateViews(direction: 'right' | 'left', steps: number = 1) {
    this.animates.forEach((x: number, index: number) => {
      ...
      if (mov) {
        // Safely assert that direction will always be 'right' or 'left'
        const animations = (mov[direction as 'right' | 'left'] || []).map(
          (m) => {
            ...
            return animate(
              this.timer / mov[direction as 'right' | 'left'].length + 'ms',
              style(applyStyle)
            );
          }
        );
        ...
        if (item) {
          ...
          this.player.onDone(() => {
            this.animates[index] =
              mov[direction as 'right' | 'left'][
                mov[direction as 'right' | 'left'].length - 1
              ];
          });
          this.player.play();
        }
